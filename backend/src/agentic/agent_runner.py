...
def _build_system_prompt(config: Dict[str, Any], tool_names: set) -> str:
    ...
    formatting_block = (
        ...
        "for that section. Each major idea gets its own section header + "
        "body block.\n"
        ...
        "- Equations and standalone mathematical expressions must always use "
        "display math: wrap them in `$$` on their own line so they render "
        "centred in the chat. Inline math uses single `$`. Never write "
        "equations as plain ASCII.\n"
