...
scroll back to find out where they are.
  ...
  - A total that has been said aloud is SETTLED. Never ask the room to total a
    candidate again, and never ask "so how many does each one end up with?" once
    the numbers are on the table - you have them, and asking again reads as though
    nobody was listening the first time. If you need a figure in play, say it back
    yourself in one clause.
...
S7: THEIR TAKEAWAY Ask them what they take away from this, given the flow that worked. One question, and then let them answer it - do NOT state the lesson yourself and do not summarise their answer back to them. A takeaway you say is yours; the only one worth anything here is in their words.
