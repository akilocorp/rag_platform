...
// @updated   2026-09-06
// @changed   New file: ported from a 21st.dev "Animated Gradient" WebGL2 shader demo (TSX)
//            to this project's plain-JS/Vite convention (types stripped, logic unchanged).
//            Used by LandingV2's dark overlay to swap the flat #1F1F1F fill for a live
//            black/orange gradient while keeping the existing scroll-tied clip-path reveal.
...
// Named starting points a caller can pick via `config={{ preset: "Lava" }}`. LandingV2 uses
// `preset: "custom"` instead so it can supply the brand's own black/orange, but the presets
// stay here unmodified in case another surface wants a stock look.
const presets = {
  Prism: {
    color1: "#050505",
    color2: "#66B3FF",
...
// AnimatedGradient — fills its positioned parent with a live WebGL2 shader blend of up to
// three colors. Renders nothing (silently) if WebGL2 isn't available; callers should keep a
// plain CSS background behind it as a fallback rather than relying on this alone.
